 ...
 *  
 * @author Graham Klyne
 * @version $Id: shuffl.js 822 2010-06-08 14:06:39Z gk-google@ninebynine.org $
 * 
...
shuffl.menuOpenWorkspace = function ()
{
    ...
    // Create selectfile card with approriate fields
    var select = shuffl.card.selectfile.newCard(
        "shuffl-selectfile",      // type
        "stock-default",          // css style
        "open-workspace",         // id
        { 'shuffl:title':   "Open workspace"
        , 'shuffl:fileuri': ""
        });
    select.css("zIndex", 3000);   // To play with modal box

    // On clicking close button, load new workspace from specified URI,
    // and hide modal box
    select.modelBind("shuffl:closeUri", function(_event, val)
        {
            var newuri = val.newval.toString();
    ...
    // On hiding modal box, remove file selector and clean up
    ...
    // Insert file selector into modal box, and display
    ////jQuery("#modal_box").children().remove();
    jQuery("#modal_box")
      .jqm({modal: false, onHide: closeBox, trigger: false})
      .jqmShow()
      .append(select);
    var resizefn = shuffl.resizeHandler(
        select, select.data("resizeAlso"), select.data("redrawFunc"));
    select.data("resizeFunc", resizefn);
    if (resizefn) { select.bind('resize', resizefn); };
    select.css({left:30, top:20}).css('position', 'absolute');
...
jQuery(document).ready(function()
{
    ...
    jQuery("#stockbar").append(shuffl.ModalBoxHTML);
